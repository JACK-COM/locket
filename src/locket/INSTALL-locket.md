...
- **Removal.** `locket uninstall` prints what it will remove and asks once (`--dry-run` only prints, `--purge` deletes each store's manifest too): the command link, `~/.locket/`, every cache, the Claude Code hook entries and the Desktop server, keeping a copy of each config it edits. Hermes hook lines are YAML and stay for you to remove; a Homebrew install ends with `brew uninstall locket`.
